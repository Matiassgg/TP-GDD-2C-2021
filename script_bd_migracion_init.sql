...
-- sp para borrar todo, no es necesario a prori pero facilita los tests
IF OBJECT_ID('[N&M''S].sp_borrado_total') IS NOT NULL
    DROP PROCEDURE [N&M'S].sp_borrado_total
GO

CREATE PROCEDURE [N&M'S].sp_borrado_total
AS
	BEGIN
		-- A medida que agreguemos las tablas, hay que cambiar el orden que se hace el DROP TABLE por el chequeo referencial que tienen
...
DECLARE @id_test BIT = 0 -- esto despues se volara
--------------------------------------------------- 
-- CHEQUEO Y CREACION DE TABLAS
---------------------------------------------------

EXEC [N&M'S].sp_borrado_total
