...
BEGIN
    ...
    SELECT
        @costo_materiales_x_tarea = SUM(ISNULL(M.precio,0))
    ...
    -- Tareas que se hizo ese camion en ese taller
    GROUP BY tarea_id

    -- Costo MO
    SELECT
        @costo_MO = SUM((M.costo_x_hora * 8) * TxO.tiempo_ejecucion)
    -- costo total de mano de obra x camion, tiempo de ejecucion siempre es 2
    FROM [N&M'S].Orden_de_Trabajo OT
        INNER JOIN [N&M'S].Tarea_x_Orden TxO ON TxO.nro_orden = OT.nro_orden
        INNER JOIN [N&M'S].Mecanico M ON m.mecanico_id = TxO.mecanico_id
    WHERE OT.camion_id = @camion_id AND OT.taller_id = @taller_id
    -- 1 camion es atendido en un solo taller x varios mecanicos
...
*/
CREATE FUNCTION [N&M'S].fn_calcular_facturacion_recorrido( @recorrido_id INT, @camion_id INT,@fechaFin DATETIME2)  AS
BEGIN
    ...
    Where V.recorrido_id = @recorrido_id and V.camion_id =  @camion_id and V.fechaFin = @fechaFin
...
BEGIN
    ...
    SELECT id_tarea_x_nro_orden, nro_orden, tarea_id, mecanico_id, fecha_inicio_planificada, fecha_inicio_real, fecha_fin_real, tiempo_ejecucion
...
FROM [N&M'S].bi_Orden_de_Trabajo OT
    ...
    INNER JOIN [N&M'S].Chofer CH ON CH.chofer_id = C.chofer_id
GROUP BY 
	[N&M'S].fn_obtener_id_tiempo(OT.fecha_generada),
    OT.camion_id,
    OT.nro_orden,
    ...
    [N&M'S].fn_calcular_costo_mantenimiento(OT.camion_id,OT.taller_id,OT.fecha_generada), --obtengo el gasto de un camion en una taller de una fecha en particular
    CH.costo_x_hora,
    [N&M'S].fn_calcular_facturacion_recorrido(V.recorrido_id, OT.camion_id, OT.fecha_generada), --obtengo la facutacion de un recorido en una fecha en particular
    [N&M'S].fn_calcular_costo_viaje(V.consumo_combustible, CH.costo_x_hora),
    [N&M'S].fn_calcular_ingresos(OT.camion_id,V.recorrido_id)
	END TRY
	BEGIN CATCH
...
EXEC sp_cargar_bi_Orden_de_Trabajo
EXEC sp_cargar_bi_Tarea_x_Orden
...
AS
    ...
    FROM [N&M'S].bi_reparaciones_camiones RC
        inner join Bi_Tiempo T on T.tiempo_id = RC.tiempo_id
...
--4
CREATE VIEW [N&M'S].vw_tareas_x_modelo
AS
    SELECT DISTINCT TOP 5
        M.descripcion 'modelo', T.nombre 'tarea', RC.cantidad_veces_realizadas
    FROM [N&M'S].bi_reparaciones_camiones RC
...
--6
CREATE VIEW [N&M'S].vw_facturacion_total_por_recorrido_por_cuatrimestre
AS
select CGC.recorrido_id, T.cuatrimestre, sum(CGC.facturacion_recorrido)  from bi_costos_ganancias_camiones CGC
inner join Bi_Tiempo T on T.tiempo_id = CGC.tiempo_id
...
AS
select CGC.edad_id, E.rango_edad,avg(CGC.costo_chofer)  from bi_costos_ganancias_camiones CGC
inner join bi_Edad E on E.edad_id = CGC.edad_id
